<template>
  <div class="city-container" v-if="!favMode">
    <div class="input-container">
      ...
      <ul class="city-list" v-if="mapboxSearchResults">
        <p class="city-list__item" v-if="searchError">
    ...
    <div class="controls">
      <button class="button-control" @click="deleteCard()">
  ...
  <div class="controls-fav" v-else>
    <button class="button-control" @click="removeFav(cityName)">
  ...
  <div
    ...
    :class="{
      loading: isLoading,
      'info-window--fav': checkIfFavorite() && favMode
    }"
    v-if="dataLoaded"
  >
    ...
    <p class="" v-if="!isLoading">{{ Math.round(current.temp) }}&deg;</p>
  ...
  <div class="info-window" :class="{ loading: isLoading }" v-else>
    <Preloader />
  </div>
        ...
        <div class="hours" v-if="dataLoaded && weekIsActive">
          <div class="hours__container">
            <div class="hours__box hours__box--weekly">
              <div v-for="day in daily" :key="day.dt" class="hours__item">
                <p class="hours__item-text">
                  {{
...
const keyForOpenWeather1 = import.meta.env.VITE_API_KEY_WEATHER
const apiKeyForIP = import.meta.env.VITE_API_KEY_IP
const mapboxAPIKey = import.meta.env.VITE_API_KEY_MAPBOX
...
checkIfFavorite.value = () => {
  if (JSON.parse(localStorage.getItem('savedCities'))) {
    return JSON.parse(localStorage.getItem('savedCities')).some(
      city => city.city === cityName.value
    )
  } else {
    return false
  }
...
const getCityByIp = async () => {
  try {
    ...
    const locationData = await axios.get(
      `https://api.ipgeolocation.io/ipgeo?apiKey=${apiKeyForIP}&ip=${IpData.data.ip}`
    )
